#!/bin/bash
# Builds a release DMG that opens on any Mac without Gatekeeper warnings: signs NoType.app with a
...
profile="${NOTARY_PROFILE:-NoType}"
app="build/DerivedData/Build/Products/Release/NoType.app"
dmg="build/NoType.dmg"

make build CONFIG=Release

# The app has a single executable, so signing the bundle covers everything inside it.
codesign --force --timestamp --options runtime \
    --entitlements NoType/Resources/NoType.entitlements --sign "$DEVELOPER_ID" "$app"
